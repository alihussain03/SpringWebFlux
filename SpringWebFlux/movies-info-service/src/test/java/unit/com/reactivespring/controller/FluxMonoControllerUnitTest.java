...
import com.reacitvespring.controller.FluxAndMonoController;
...
public class FluxMonoControllerUnitTest {
  ...
  void flux_approach2() {
    var flux = webTestClient.get().uri("/flux").exchange().expectStatus().is2xxSuccessful().returnResult(Integer.class)
        .getResponseBody();
    StepVerifier.create(flux).expectNext(1, 2, 3).expectComplete();
  }
  ...
  void stream() {
    ...
    StepVerifier.create(flux).expectNext(0, 1, 2).thenCancel().verify();
  }
